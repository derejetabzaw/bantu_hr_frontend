...
class ClientConnection(object):
    ...
    def receive(self, verify_checksum=True, verify_size=False):
        ...
        while len(recv) < temp.size:
            ...
            temp_data = self._socket.recv(
                buffer_size if rem_size > buffer_size else rem_size)
            if not temp_data:
                raise Exception("connection is closed by server")
            recv += temp_data
        data = header + recv
    ...
    def verify_response(self, cmd_name: list = None):
        ...
        # print("Device_CMD", device_cmd)
        # if self._response.cmd not in [device_cmd[x] for x in cmd_name]:
        #     raise Exception(
        #         "unexpected response cmd: {}".format(self._response.cmd))
...
class ZMM220_TFT(ClientConnection):
    ...
    def get_fp_data(self, user_id: int, finger_id: int, disable_device=True):
        ...
        if disable_device:
            self.disable_device()
        try:
            self.send_cmd("get_fp", struct.pack("<HB", user_id, finger_id), [
                          "recv_buff_header", "nak", "no_data"])
    ...
    def send_file(self, file_name: str, file_data: bytes, disable_device=True):
        ...
        try:
            ...
            file_name = file_name + \
                struct.pack("<{}x".format(39 - len(file_name)))
    ...
    def del_user_pic(self, person_id: str, disable_device=True):
        ...
        if disable_device:
            self.disable_device()
        try:
            self.send_cmd("del_user_pic", person_id.encode(
                self._encoding) + b".jpg\x00", ["ack", "no_pic"])
    ...
    def get_user_face(self, person_id: str, face_id=50, disable_device=True):
        ...
        try:
            ...
            payload = person_id + \
                struct.pack("<{}xxB2x".format(24 - len(person_id)), face_id)
            self.send_cmd("get_face", payload, [
                          "recv_buff_header", "nak", "no_data"])
            if self._response.cmd == device_cmd["nak"]:
                return b''
            elif self._response.cmd == device_cmd["no_data"]:
    ...
    def set_user_face(self, person_id: str, face_data: bytes, face_index=50, disable_device=True):
        ...
        try:
            ...
            person_id = person_id.encode(self._encoding)
            payload = person_id + \
                struct.pack("<{}xxBH".format(24 - len(person_id)),
                            face_index, len(face_data))
    ...
    def set_fp(self, fp_info: models.FPInfo, disable_device=True):
        ...
        try:
            ...
            #     raise Exception("problem in data sending")
            payload = struct.pack(
                "<HBBH", fp_info.user_id, fp_info.finger_id, fp_info.enabled, len(fp_info.data))
    ...
    def set_fps(self, fp_infos: list, disable_device=True):
        ...
        try:
            for fp_info in fp_infos:
                self.send_cmd("recv_buff_header", struct.pack(
                    "<I", len(fp_info.data)))
    ...
    def get_fp(self, user_id: int, finger_id: int, disable_device=True):
        ...
        if disable_device:
            self.disable_device()
        try:
            self.send_cmd("get_fp_ex", struct.pack("<HB", user_id, finger_id), [
                          "recv_buff_header", "nak", "no_data"])
            if self._response.cmd == device_cmd["nak"]:
                return
            elif self._response.cmd == device_cmd["no_data"]:
    ...
    def _get_data_buffer(self, data_id: int, disable_device=True):
        ...
        try:
            ...
            self.send_cmd("start_buff_stream", payload, [
                          "ack", "nak", "recv_buff_content", "no_record"])
            if self._response.cmd == device_cmd["recv_buff_content"]:
                ...
                for _ in range(0, packets):
                    ...
                    payload = struct.pack("<I", data_size - rem_size) + \
                        struct.pack("<I", rem_size if rem_size <
                                    packet_size else packet_size)
                    self.send_cmd("buff_stream", payload, ["recv_buff_header"])
                    recv_size = struct.unpack(
                        "<I", self._response.payload[:4])[0]
                    self.receive()
                    self.verify_response(["recv_buff_content"])
                    data = self._response
                    self.receive()
